...
export default function useList<T = any, R = any>(
  props: UseListProps<T, R>,
): {
  setParams: Function;
  list: Array<T>;
  params: Params<R>;
  ...
  const [
    isHistory,
    { setTrue: setIsHistoryTrue, setFalse: setIsHistoryFalse },
  ] = useBoolean(!!props.history);
  const [
    tableLoading,
  ...
  useEffect(() => {
    if (isHistory) {
      ...
      window.history.replaceState({}, `history_${Date.now()}`, url);
    }
  }, [isHistory, params]);
